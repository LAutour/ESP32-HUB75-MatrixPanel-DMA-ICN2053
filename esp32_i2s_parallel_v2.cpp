...
esp_err_t i2s_parallel_driver_install(i2s_port_t port, i2s_parallel_config_t* conf) 
{
  ...
  uint32_t clk_div_main = I2S_PARALLEL_CLOCK_HZ / conf->sample_rate / i2s_parallel_get_memory_width(port, (i2s_parallel_cfg_bits_t)conf->sample_width);
  ...
  //Serial.println(F("2"));
  // Setup GPIOs
  int bus_width = get_bus_width((i2s_parallel_cfg_bits_t)conf->sample_width);
